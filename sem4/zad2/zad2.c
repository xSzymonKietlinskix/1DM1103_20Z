...
int main(int argc, char *argv[]) {
    ...
    A = fopen("A.txt", "r"); // otwieranie pliku a.txt
    x = fopen("x.txt", "r"); // otwieranie pliku x.txt
    ...
    for(i = 0; i < kolumny; i++)
    {
        for(j = 0; j < wiersze; j++)
        {
            ...
            z = 0;
            x = fopen("x.txt", "r");
            for(int g = 0; g < j + 2; g++) // służy do przeskakiwania na odpowiednią daną w pliku x.txt
            {
